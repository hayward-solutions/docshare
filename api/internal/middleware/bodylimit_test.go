...
func TestSmallBodyLimitForNonUploadRoutes(t *testing.T) {
	...
	cases := []struct {
		...
		{"small body to JSON route passes", "/api/auth/login", 256, http.StatusOK},
		{"oversize body to JSON route is rejected", "/api/auth/login", 4096, http.StatusRequestEntityTooLarge},
		{"oversize body to presign is rejected", "/api/files/upload/presign", 4096, http.StatusRequestEntityTooLarge},
